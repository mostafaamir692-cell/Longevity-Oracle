      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
          ...
          <FadeIn className="lg:col-span-2 h-full">
            <div className="relative h-full min-h-[340px] rounded-3xl bg-[#040b14] border border-border overflow-hidden p-10 flex flex-col justify-between">
              <div className="absolute -top-20 -left-20 w-72 h-72 bg-primary/10 rounded-full blur-3xl pointer-events-none" />
              <div className="absolute -bottom-10 -right-10 w-48 h-48 bg-gold/8 rounded-full blur-3xl pointer-events-none" />
              ...
              <div className="relative z-10">
                ...
                <p className="text-foreground/45 text-base font-light leading-relaxed max-w-lg">
              ...
              <div className="relative z-10 mt-8 pt-6 border-t border-white/8">
                <p className="text-sm font-display italic text-foreground/50">
                  "FOY is more than a clinic. It is the beginning of a new approach to modern healthcare."
                </p>
                <p className="text-xs text-primary mt-2 font-medium tracking-wide text-glow">— Dr. Ahmed Amer, Founder</p>
              </div>
            </div>
          </FadeIn>
          ...
          <FadeIn delay={0.1} className="h-full">
            <div className="h-full min-h-[340px] rounded-3xl bg-primary/5 border border-primary/20 p-8 flex flex-col justify-between">
              <div>
                ...
                {[
                  ...
                  <div key={stat.label} className="flex items-baseline justify-between py-4 border-b border-primary/10 last:border-0">
                    <span className="text-2xl font-display font-bold text-primary text-glow">{stat.num}</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {pillars.map((p, idx) => (
            <FadeIn key={idx} delay={idx * 0.08} direction="up" className="h-full">
              <div className="group h-full rounded-3xl bg-card border border-border p-7 hover:border-primary/40 hover:shadow-[0_8px_32px_rgba(0,0,0,0.4),0_0_20px_rgba(16,185,171,0.06)] hover:-translate-y-1.5 transition-all duration-300 cursor-default">
                <div className="w-10 h-10 rounded-2xl bg-primary/8 border border-primary/15 flex items-center justify-center text-primary mb-5 group-hover:bg-primary/20 group-hover:border-primary/40 group-hover:text-glow transition-all duration-300">
                  {p.icon}
                </div>
                <h3 className="font-bold text-sm text-foreground/90 mb-2 leading-snug">{p.title}</h3>
